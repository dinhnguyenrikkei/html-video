...
class OnnxV3LiteEngine:
    ...
    def __init__(
        ...
        **_kw,
        ...
        so.enable_cpu_mem_arena = False  # Free RAM immediately, prevent BFCArena allocation spikes
        if threads and threads > 0:
            so.intra_op_num_threads = threads
            
        # Dynamically support CUDA GPU Execution Providers if available (avoiding DML driver resets)
        available_providers = ort.get_available_providers()
        prov = []
        if "CUDAExecutionProvider" in available_providers:
            prov.append("CUDAExecutionProvider")
        prov.append("CPUExecutionProvider")
        self.providers = prov  # store for encoder session
    ...
    def _encode_ref(self, ref_audio_path: str) -> np.ndarray:
        ...
        if self._sess_codec_enc is None:
            import onnxruntime as ort
            self._sess_codec_enc = ort.InferenceSession(self._codec_enc_path, providers=getattr(self, "providers", ["CPUExecutionProvider"]))
