...
export async function render(input: RenderInput, ctx: RenderContext): Promise<RenderOutput> {
  ...
  try {
    ...
    // recordVideo starts capturing the moment the page exists, so this is
  ...
  // of the front of multi-composition webms. Back off 120ms so rounding /
  ...
  // Cap at 5s: if browser startup took longer than that something is wrong and
  // we should not trim that far into real content.
  if (leadInMs > 5000) {
    console.warn(`[hyperframes] leadInMs=${leadInMs}ms exceeds 5s — clamping seekSec to 5.0s to avoid cutting real content.`);
  }
  const seekSec = leadInMs > 0 ? Math.min(leadInMs / 1000, 5.0) : 0;
